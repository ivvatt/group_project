...
if __name__ == "__main__":
    header = read_header()
    middle_ages_death = [] #create an empty list for the middle ages
    for year in range(400, 1300): #middle ages years
        ...
        if os.path.exists('years/'+padded_year): 
            ...
            for person in data:
                if 'deathCause_label' in person and 'description' in person: #filter for death cause
                    #if re.findall('(?i)poison', person['deathCause_label']):
                    middle_ages_death.append(person) #add filtered info to list
#save in a json file (list of dictionaries)
with open('middle_ages_death.json', 'w', encoding='utf-8') as file: #w = writing
    json.dump(middle_ages_death, file, indent=4)
        ...
        if os.path.exists('years/'+padded_year): 
            ...
            for person in data:
                if 'deathCause_label' in person and 'description' in person:
                    #if re.findall('(?i)decapitation', person['deathCause_label']) or re.findall('(?i)hanging', person['deathCause_label']):
                    renaissance_death.append(person)
#save in a json file
with open('renaissance_death.json', 'w', encoding='utf-8') as file: 
    json.dump(renaissance_death, file, indent=4)
'''
...
if __name__ == "__main__":
    header = read_header()
    enlight_industrial_death_noother = []
                ...
                if 'deathCause_label' in person and 'deathYear' in person and 'description' in person: 
                    ...
                    for deathCause in causes:
                        if re.search('(cancer|tumor|leukemia|\w+oma)($|\W)', deathCause, flags=re.I):
                            ...
                            #enlight_industrial_death_noother.remove('other_diseases')                                                                                   
                        if person['deathCause_label'] != 'other_diseases':
                            enlight_industrial_death_noother.append(person.copy())
                        
#save in a json file
with open('enlight_industrial_death_noother.json', 'w', encoding='utf-8') as file: 
    json.dump(enlight_industrial_death_noother, file, indent=4)

#selecting data for the modern era 
if __name__ == "__main__":
    header = read_header()
    modern_death_noother = []
            ...
            for person in data:
                if 'deathCause_label' in person and 'description' in person:
                    ...
                    for deathCause in causes:
                        ...
                        if person['deathCause_label'] != 'other_diseases':                                                                                
                            modern_death_noother.append(person.copy())
                        
#save in a json file
with open('modern_death_noother.json', 'w', encoding='utf-8') as file: 
    json.dump(modern_death_noother, file, indent=4)
'''
#make it into a csv file
#middle ages
with open('middle_ages_death.csv', 'w', newline='', encoding='utf-8') as file: 
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
    for person in middle_ages_death:
        filewriter.writerow([person['birthYear'], person['birthDate'], person['deathYear'], person['deathCause_label'], person['description']])

#renaissance
with open('renaissance_death.csv', 'w', newline='', encoding='utf-8') as file:
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
    for person in renaissance_death:
        filewriter.writerow([person['birthYear'], person['birthDate'], person['deathYear'], person['deathCause_label'], person['description']])
'''
#enlightenment + industrial revolution
with open('enlight_industrial_death_noother.csv', 'w', newline='', encoding='utf-8') as file:
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
    for person in enlight_industrial_death_noother:
        filewriter.writerow([person['birthYear'], person['birthDate'], person['deathYear'], person['deathCause_label'], person['description']])

#modern
with open('modern_death_noother.csv', 'w', newline='', encoding='utf-8') as file:
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
    for person in modern_death_noother:
        filewriter.writerow([person['birthYear'], person['birthDate'], person['deathYear'], person['deathCause_label'], person['description']])