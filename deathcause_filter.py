            ...
            for person in data:
                if 'deathCause_label' in person and 'deathYear' in person and 'description' in person: 
                    for deathCause in person['deathCause_label'][1:-1].split('|'):
                        ...
                        enlight_industrial_death.append(person.copy())
#save in a json file
with open('enlight_industrial_death.json', 'w', encoding='utf-8') as file: 
    json.dump(enlight_industrial_death, file, indent=4)
                ...
                if 'deathCause_label' in person and 'description' in person:
                     for deathCause in person['deathCause_label'][1:-1].split('|'):
                        if re.search('(cancer|tumor|leukemia|\w+oma)($|\W)', deathCause, flags=re.I):
                            ...
                            person['deathCause_label'] = 'other_diseases'                                                                                     
                        modern_death.append(person.copy())
#save in a json file
with open('modern_death.json', 'w', encoding='utf-8') as file: 
    json.dump(modern_death, file, indent=4)
...
with open('enlight_industrial_death.csv', 'w', newline='', encoding='utf-8') as file:
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
    for person in enlight_industrial_death:
        filewriter.writerow([person['birthYear'], person['birthDate'], person['deathYear'], person['deathCause_label'], person['description']])

#modern
with open('modern_death.csv', 'w', newline='', encoding='utf-8') as file:
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
    for person in modern_death:
        filewriter.writerow([person['birthYear'], person['birthDate'], person['deathYear'], person['deathCause_label'], person['description']])