...
#selecting data for actors during the middle ages
if __name__ == "__main__":
    header = read_header()
    actors = [] #create an empty list for actors
    for year in range(400, 2015): # during middle ages
            ...
            for person in data:
                if 'deathCause_label' in person: #filter for death cause
                    if 'description' in person: #filter for profession
                        if 'actor' in person['description']:
                            actors.append(person) #add filtered info to list
