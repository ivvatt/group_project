...
'''
#selecting data for actresses
if __name__ == "__main__":
    header = read_header()
    actresses = [] #create an empty list for actors
    for year in range(400, 2015): 
        padded_year = ('0000' + str(year))[-4:] #add 0's in front of numbers
        if os.path.exists('years/'+padded_year): 
            data = process_csv(padded_year, header)
            for person in data:
                if 'deathCause_label' in person and 'description' in person:
                    causes = person['deathCause_label'][1:-1].split('|') if '{' in person['deathCause_label'] else [person['deathCause_label']]
                    for deathCause in causes:
                        if 'actress' in person['description'].lower():
                            if re.search('(cancer|tumor|leukemia|\w+oma)($|\W)', deathCause, flags=re.I):
                                person['deathCause_label'] = 'cancer'
                            elif 'bronch' in deathCause.lower() or 'asthma' in deathCause.lower() or 'lung' in deathCause.lower() or 'pulmon' in deathCause.lower() or 'respirat' in deathCause.lower() or 'tubercolosis' in deathCause.lower():
                                person['deathCause_label'] = 'other_respiratory_problems' 
                           ...
                           #modern_death_noother.remove('other_diseases') 
                            if person['deathCause_label'] != 'other_diseases':                                                                                
                                actresses.append(person.copy())
#save in a json file (list of dictionaries)
with open('actresses.json', 'w', encoding='utf-8') as file: #w = writing
    json.dump(actresses, file, indent=4)
#make csv file
with open('actresses.csv', 'w', newline='', encoding='utf-8') as file:
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
        ...
        if os.path.exists('years/'+padded_year): 
            ...
            for person in data:
                if 'deathCause_label' in person and 'description' in person:
                    causes = person['deathCause_label'][1:-1].split('|') if '{' in person['deathCause_label'] else [person['deathCause_label']]
                    for deathCause in causes:
                        if 'politician' in person['description'].lower() or 'minister' in person['description'].lower() or 'diplomat' in person['description'].lower() or 'parliament' in person['description'].lower()\
                         or 'senat' in person['description'].lower() or 'royalty' in person['description'].lower() or 'statesm' in person['description'].lower() or 'count' in person['description'].lower() or 'congress' in person['description'].lower()\
                         or 'baron' in person['description'].lower():
                            if re.search('(cancer|tumor|leukemia|\w+oma)($|\W)', deathCause, flags=re.I):
                                person['deathCause_label'] = 'cancer'
                            elif 'bronch' in deathCause.lower() or 'asthma' in deathCause.lower() or 'lung' in deathCause.lower() or 'pulmon' in deathCause.lower() or 'respirat' in deathCause.lower() or 'tubercolosis' in deathCause.lower():
                                person['deathCause_label'] = 'other_respiratory_problems' 
...
#selecting data for writers
if __name__ == "__main__":
    header = read_header()
    writers = [] #create an empty list for actors
    for year in range(400, 2015): 
        padded_year = ('0000' + str(year))[-4:] #add 0's in front of numbers
        if os.path.exists('years/'+padded_year): 
            data = process_csv(padded_year, header)
            for person in data:
                if 'deathCause_label' in person and 'description' in person:
                    causes = person['deathCause_label'][1:-1].split('|') if '{' in person['deathCause_label'] else [person['deathCause_label']]
                    for deathCause in causes:
                        if 'writer' in person['description'].lower() or 'author' in person['description'].lower() or 'poet' in person['description'].lower() or 'scribe' in person['description'].lower():
                            if re.search('(cancer|tumor|leukemia|\w+oma)($|\W)', deathCause, flags=re.I):
                                person['deathCause_label'] = 'cancer'
                            elif 'bronch' in deathCause.lower() or 'asthma' in deathCause.lower() or 'lung' in deathCause.lower() or 'pulmon' in deathCause.lower() or 'respirat' in deathCause.lower() or 'tubercolosis' in deathCause.lower():
                                person['deathCause_label'] = 'other_respiratory_problems' 
                           ...
                           #modern_death_noother.remove('other_diseases') 
                            if person['deathCause_label'] != 'other_diseases':                                                                                
                                writers.append(person.copy())
#save in a json file (list of dictionaries)
with open('writers.json', 'w', encoding='utf-8') as file: #w = writing
    json.dump(writers, file, indent=4)
#make csv file
with open('writers.csv', 'w', newline='', encoding='utf-8') as file:
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
    for person in writers:
        filewriter.writerow([person['birthYear'], person['birthDate'], person['deathYear'], person['deathCause_label'], person['description']])

#selecting data for athletes
if __name__ == "__main__":
    header = read_header()
    athletes = [] #create an empty list for actors
    for year in range(400, 2015): 
        padded_year = ('0000' + str(year))[-4:] #add 0's in front of numbers
        if os.path.exists('years/'+padded_year): 
            data = process_csv(padded_year, header)
            for person in data:
                if 'deathCause_label' in person and 'description' in person:
                    causes = person['deathCause_label'][1:-1].split('|') if '{' in person['deathCause_label'] else [person['deathCause_label']]
                    for deathCause in causes:
                        if 'athlete' in person['description'].lower() or 'sport' in person['description'].lower():
                            if re.search('(cancer|tumor|leukemia|\w+oma)($|\W)', deathCause, flags=re.I):
                                person['deathCause_label'] = 'cancer'
                            elif 'bronch' in deathCause.lower() or 'asthma' in deathCause.lower() or 'lung' in deathCause.lower() or 'pulmon' in deathCause.lower() or 'respirat' in deathCause.lower() or 'tubercolosis' in deathCause.lower():
                                person['deathCause_label'] = 'other_respiratory_problems' 
                           ...
                           #modern_death_noother.remove('other_diseases') 
                            if person['deathCause_label'] != 'other_diseases':                                                                                
                                athletes.append(person.copy())
#save in a json file (list of dictionaries)
with open('athletes.json', 'w', encoding='utf-8') as file: #w = writing
    json.dump(athletes, file, indent=4)
#make csv file
with open('athletes.csv', 'w', newline='', encoding='utf-8') as file:
    filewriter = csv.writer(file)
    filewriter.writerow(['birthYear', "birthDate", "deathYear", "deathCause_label", "profession"])
